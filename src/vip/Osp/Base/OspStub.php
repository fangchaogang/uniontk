...
namespace fangchaogang\uniontk\vip\Osp\Base;

//use Thrift\Transport\TFramedTransport;
//use Thrift\Type\TMessageType;
//use Thrift\Transport\TSocket;
//use Thrift\Transport\TBufferedTransport;
use fangchaogang\uniontk\vip\Osp\Context\InvocationContextFactory;
...
class OspStub
{
    ...
    {
        \fangchaogang\uniontk\vip\Osp\Log\Logger::info("[OspStub][InitInvocation][Service-Version-Method][" . $this->serviceName . "-" . $this->version . "-" . $method . "]");
        ...
        , $timestamp, $version, $request, $appSecret)
    {
        ...
        return \fangchaogang\uniontk\vip\Osp\Util\HmacUtil::hmac($sign, $appSecret);
    }
    ...
    {
        if (!$restAddr) throw new \fangchaogang\uniontk\vip\Osp\Exception\OspException("The openapi appURL is null, please set the appURL value");
    ...
    {
        $ctx = InvocationContextFactory::getInstance();
        $oprot = new \fangchaogang\uniontk\vip\Osp\Protocol\JSONProtocol(new \NiuGengYun\EasyTBK\Vip\Osp\Buffer\MemoryBuffer());
        $args->write($oprot);
        ...
        if ($appKey == null) throw new \fangchaogang\uniontk\vip\Osp\Exception\OspException("The openapi appKey is null, please set the appKey value");

        $appSecret = $ctx->getAppSecret();
        if ($appSecret == null) throw new \fangchaogang\uniontk\vip\Osp\Exception\OspException("The openapi appSecret is null, please set the appSecret value");
        ...
        $timestamp = round(\fangchaogang\uniontk\vip\Osp\Util\TimeUtil::currentTimeMillis() / 1000);
        ...
        try {
            ...
            throw new \fangchaogang\uniontk\vip\Osp\Exception\OspException(\Osp\Util\MessageUtil::getInvocationMsg($ctx, $e->getMessage()), $e);
        }
    ...
    public function receive_base($result)
    {
        ...
        if (\NiuGengYun\EasyTBK\Vip\Osp\Base\OspStub::$RETURN_NULL != $response) {
            $iprot = new \fangchaogang\uniontk\vip\Osp\Protocol\JSONProtocol(new \NiuGengYun\EasyTBK\Vip\Osp\Buffer\MemoryBuffer());
            ...
            if (!"0" == $returnCode) {
                ...
                throw new \fangchaogang\uniontk\vip\Osp\Exception\OspException($returnMessage, $returnCode);
            }
